...
public class Board
{
    ...
    { 
        ...
        int basePerType = Mathf.FloorToInt((float)totalCells / totalTypes / 3f) * 3;
        ...
        // Nếu vẫn còn ô trống, lấp bằng random loại bất kỳ nhưng đảm bảo không phá vỡ chia 3
        while (itemPool.Count < totalCells)
        {
            NormalItem.eNormalType t = allTypes[UnityEngine.Random.Range(0, allTypes.Count)];
            itemPool.Add(t);
        }

        // B3. Shuffle danh sách để trộn ngẫu nhiên
        itemPool = itemPool.OrderBy(x => UnityEngine.Random.value).ToList();

        // B4. Gán lần lượt cho từng cell trong bảng
        int index = 0;
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                // Lấy loại từ pool thay vì random trực tiếp
                NormalItem.eNormalType itemType = itemPool[index++];
        ...
        Debug.Log($"Board filled with {itemPool.Count} items. Each type count divisible by 3.");
    }
    ...
    public bool AllCellsEmpty()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            ...
            {
                Cell cell = m_cells[x, y];
                // Nếu bất kỳ cell nào còn Item thì chưa trống
                if (cell.Item != null)
                    return false;
            }
        }
        return true; // tất cả đều trống → board rỗng
    }
